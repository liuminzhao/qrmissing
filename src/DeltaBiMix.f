c===========================================================
c$$$
C$$$  Time-stamp: <liuminzhao 05/14/2014 16:45:07>
...
C------------------------------
      ...
      subroutine updategslope(x,gamma1, beta1, gamma2,
     &     beta2sp, mu1, sigma1, mu2, sigma2, omega11, omega10,
     &     omega21, omega20sp, betay, betaysp,
     &     p, tau, n, xdim, delta, K, y, r, g1, g2)
      implicit none
...
C     Tmp
      ...
      call mydelta2bisemixslope(x,gamma1, beta1, gamma2,
     &     beta2sp, mu1, sigma1, mu2, sigma2, omega11, omega10,
     &     omega21, omega20sp, betay, betaysp,
     &     p, tau, n, xdim, dd, K)
      ...
      do i = 1, n
         if (r(i) .eq. 1) then
            do j = 1, k
               prob1(j) = omega11(j)*dnrm(y(i,1), dd(i,1) +
     &              dot_product(beta1, x(i,:)) + mu1(j), sigma1(j), 0)
               prob2(j) = omega21(j)*dnrm(y(i,2), dd(i,2) +
     &              betay*y(i,1) + mu2(j) , sigma2(j), 0)
            end do
         else
